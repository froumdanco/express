...
const forget = require('../../models/confrimcode.model');
...
exports.create = async (req, res) => {
    ...
    let con = await forget.find({ parent: req.user.id }).count();
    ...
    await bg.save();
    res.render('confrim', { email: users.email, code: code, name: users.name, url: emailConfig.url }, function (err, final_html) {
        if (err) throw err; // TODO: handle errors better

        var mailOptions = {
            from: emailConfig.username,
            to: users.email,
            subject: 'Confirm mail Danco Coin',
            html: final_html
...
exports.update = async (req, res) => {
    ...
    let lastrequest = await forget.findOne({ parent: req.user.id }).sort({ 'updatedAt': 'desc' });
    if (code == undefined) {
        return res.status(422).send({
